    ...
    class CategorizationManager
    {
        ...
        public bool StartCategorization(int docSetId, ArrayList categorizationSampleDocs)
        {
            ...
            try
            {
                ...
                {
                    if (logging) Util.DWMSLog("CategorizationManager.StartCategorization", "No skip categorisation", EventLogEntryType.Warning);
                    ...
                    if (pageList.Count > 0)
                    {
                        ...
                        SaveDocs(docSetId, ref docList, appPersonalTable); // Save the docs
                    }
                }
                ...
                SetStatusEnum status = docSetDb.HasVerificationOfficerAssigned(docSetId) ? SetStatusEnum.Pending_Verification : SetStatusEnum.New;
                docSetDb.UpdateSetStatus(docSetId, status);                
                if (status == SetStatusEnum.New)
                {
                    //Added By Edward for Email of New 08.11.2013
                    Util.DWMSLog("StartCategorization ", String.Format("CategorizationManager.StartCategorization.Sending Email"), System.Diagnostics.EventLogEntryType.Information);
                    docSetDb.SendMail(docSetId);     
                }
                // Update the appication status
        ...
        private void BreakLinksSimilarPages(ref ArrayList pageList)
        {
            for (int i = 0; i < pageList.Count; i++)
            {
                    ...
                    else
                    {
                        ...
                        if (currPageOcr.PrevPage != null)
                        {
                            if (currPageOcr.DocumentType.ToUpper().Equals(currPageOcr.PrevPage.DocumentType.ToUpper()) || currPageOcr.PrevPage.DocumentType.ToUpper().Equals(DocTypeEnum.Unidentified.ToString().ToUpper()) || currPageOcr.DocumentType.ToUpper().Equals(DocTypeEnum.Unidentified.ToString().ToUpper())) // If the previous page and current page have the same doc type
                            {
                                ...
                                //}
                                #endregion
                                if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization cont page {0}: {1}", currPageOcr.DocumentType.ToString(), currPageOcr.RawPageNo.ToString()), EventLogEntryType.Warning);
                                    ...
                                    !currPageOcr.Nric.ToLower().Equals(currPageOcr.PrevPage.Nric.ToLower())) // If the previous and current pages' NRIC is not empty and equal
                                {
                                    ...
                                    currPageOcr.IsDocStartPage = true; // Set the page as the start page of the document group
                                    if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization break nric page {0}: {1}", currPageOcr.DocumentType.ToString(), currPageOcr.RawPageNo.ToString()), EventLogEntryType.Warning);
                                }
                                ...
                                else if (currPageOcr.NextPage != null)
                                {
                                    ...
                                    {
                                        if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization update unidentified {0}: {1} -> {2}", currPageOcr.RawPageNo.ToString(), currPageOcr.DocumentType.ToString(), currPageOcr.NextPage.DocumentType.ToString()), EventLogEntryType.Warning);
                                ...
                                {
                                    if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization update unidentified {0}: {1} <- {2}", currPageOcr.PrevPage.RawPageNo.ToString(), currPageOcr.PrevPage.DocumentType.ToString(), currPageOcr.DocumentType.ToString()), EventLogEntryType.Warning);
                            ...
                            {
                                if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization jump page {0}: {1}", currPageOcr.DocumentType.ToString(), prePageOcr.PrevPage.DocumentType.ToString()), EventLogEntryType.Warning);
                                ...
                                {
                                    if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization match {0}: {1}", currPageOcr.DocumentType.ToString(), prePageOcr.PrevPage.DocumentType.ToString()), EventLogEntryType.Warning);
                                    ...
                                    {
                                        if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization try merge {0}: {1}", currPageOcr.DocumentType.ToString(), currPageOcr.RawPageNo.ToString()), EventLogEntryType.Warning);
                            ...
                            {
                                if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization break page {0}: {1} <-> {2}: {3}", currPageOcr.PrevPage.DocumentType.ToString(), currPageOcr.PrevPage.RawPageNo.ToString(), currPageOcr.DocumentType.ToString(), currPageOcr.RawPageNo.ToString()), EventLogEntryType.Warning);
                                currPageOcr.PrevPage.NextPage = null; // Break the link of the previous page to this page
                                currPageOcr.PrevPage = null; // Break the link of this page to the previous page
                        ...
                        {
                            if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization start page {0}: {1}", currPageOcr.DocumentType.ToString(), currPageOcr.RawPageNo.ToString()), EventLogEntryType.Warning);
                            bool startPage = true;
                            ...
                            if (i > 0)
                            {
                                ...
                                for (int cnt = i - 1; cnt >= 0; cnt--)
                                {
                                    ...
                                    if (!prevPageOcr.IsBlank && !currPageOcr.IsBlank)
                                    {
                                        ...
                                        {
                                            if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization try merge {0}: {1} <- {2}: {3}", prevPageOcr.DocumentType.ToString(), prevPageOcr.RawPageNo.ToString(), currPageOcr.DocumentType.ToString(), currPageOcr.RawPageNo.ToString()), EventLogEntryType.Warning);
                                        ...
                                        {
                                            if (logging) Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization same type merge {0}: {1} & {2}: {3}", prevPageOcr.DocumentType.ToString(), prevPageOcr.RawPageNo.ToString(), currPageOcr.DocumentType.ToString(), currPageOcr.RawPageNo.ToString()), EventLogEntryType.Warning);
                                            prevPageOcr.NextPage = currPageOcr;
                                            currPageOcr.PrevPage = prevPageOcr;
                                            startPage = false;
                                        }
                ...
                {
                    Util.DWMSLog("CategorizationManager.BreakLinksSimilarPages", String.Format("Categorization try catch {0}: {1}", currPageOcr.RawPageNo.ToString(), currPageOcr.DocumentType.ToString()), EventLogEntryType.Error);
                }
            }
        }
                ...
                if (currPageOcr.DocumentType.Equals(DocTypeEnum.HLE.ToString()) && currPageOcr.NextPage == null)
                {
                    for (int y = i + 1; y < pageList.Count; y++)
                    {
                            ...
                            nextPageOcr.IsDocStartPage)
                        {
                            ...
                            //{
                            if (logging) Util.DWMSLog("CategorizationManager.LinkSimilarHlePagesNotInSequence", String.Format("Categorization same type merge {0}: {1} & {2}: {3}", currPageOcr.DocumentType.ToString(), currPageOcr.RawPageNo.ToString(), nextPageOcr.DocumentType.ToString(), nextPageOcr.RawPageNo.ToString()), EventLogEntryType.Warning);
        ...
        /// </summary>
        /// <param name="docList"></param>
        private void SaveDocs(int docSetId, ref ArrayList docList, AppPersonal.AppPersonalDataTable appPersonalTable)
        {
            // Save documents
            DocSetDb docSetDb = new DocSetDb();
            ...
            switch (channel)
            {
                ...
                case "Mixed":
                    docChannel = "009";
                    break;
                case "MyHDBPage Common Panel":  //Added By Edward 31.10.2013 011 New Channel
                    docChannel = "O11";
                    break;
                default:
                    docChannel = "009";
